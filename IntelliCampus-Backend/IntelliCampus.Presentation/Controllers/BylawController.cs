...
public class BylawController : ControllerBase
{
    ...
    [HttpPost("{id}/validate-course-mapping")]
    public async Task<ActionResult<CourseMappingValidationResultDto>> ValidateCourseMapping(
        int id, [FromBody] ValidateCourseMappingDto dto)
    {
        var result = await _bylawService.ValidateCourseMappingAsync(id, dto);
        return Ok(result);
    }
