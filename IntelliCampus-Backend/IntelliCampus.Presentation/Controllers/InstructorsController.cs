...
public class InstructorsController : ControllerBase
{
    ...
    [HttpGet("available-for-loan")]
    public async Task<ActionResult<IEnumerable<InstructorDto>>> GetAvailableForLoan([FromQuery] int facultyId)
    {
        var instructors = await _instructorService.GetInstructorsForLoanAsync(facultyId);
        return Ok(instructors);
    }

    [HttpPut("{id}/loan")]
    public async Task<ActionResult<InstructorDto>> LoanInstructor(int id, [FromBody] UpdateInstructorDto dto)
    {
        var instructor = await _instructorService.LoanInstructorAsync(id, dto);
        return Ok(instructor);
    }
