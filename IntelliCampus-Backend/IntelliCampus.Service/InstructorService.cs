...
public class InstructorService(IUnitOfWork unitOfWork, IPasswordService passwordService, ICodeGenerationService codeGeneration, UrlResolver urlResolver, ICurrentAdminContext adminContext) : IInstructorService
{
    ...
    public async Task<IEnumerable<InstructorDto>> GetInstructorsForLoanAsync(int facultyId)
    {
        var spec = new InstructorSpec(facultyId, availableForLoan: true);
        var instructors = await Instructors.GetAllAsync(spec, asNoTracking: true);
        return instructors.Select(MapToDto);
    }

    public async Task<InstructorDto> LoanInstructorAsync(int instructorId, UpdateInstructorDto dto)
    {
        var spec = new InstructorSpec(instructorId);
        var instructor = await Instructors.GetByIdAsync(spec);
        ...
        var hostFacultyId = await _adminContext.GetFacultyIdAsync();

        instructor.User.FacultyId = hostFacultyId;
        instructor.Status = InstructorStatus.Loan;
        ...
        var loanInstructor = await _unitOfWork.GetRepository<LoanInstructor, int>().GetByIdAsync(instructor.UserId);
        if (loanInstructor is null)
        {
            loanInstructor = new LoanInstructor
            {
                UserId = instructor.UserId,
                LoanFromFacultyId = dto.LoanFromFacultyId,
                LoanFromDepartmentId = dto.LoanFromDepartmentId,
