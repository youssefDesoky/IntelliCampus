...
public class CourseService(IUnitOfWork unitOfWork, UrlResolver urlResolver, IExcelImportService excelImportService, ICurrentAdminContext adminContext) : ICourseService
{
    ...
    public async Task<PaginatedResult<CourseDto>> GetActiveCoursesByStudentBylawAsync(int studentId, CourseQueryParams queryParams)
    {
        ...
        var electiveCourseIds = bylawCourses
            ...
            .ToHashSet();
        var bylawPrereqLookup = bylawCourses
            .Where(bc => bc.Prerequisites?.Count > 0)
            .ToDictionary(bc => bc.CourseId, bc => bc.Prerequisites!);

        if (bylawCourseIds.Count == 0)
            return new PaginatedResult<CourseDto>(queryParams.PageIndex, 0, 0, new List<CourseDto>());

        var completedOnlyIds = (await StudentCourses.GetAllAsync(
            new StudentCourseIdsSpec(studentId, [StudentCourseStatus.Completed]),
            asNoTracking: true))
            .Select(sc => sc.CourseId)
            .ToHashSet();

        var availableCourseIds = bylawCourseIds.Where(id => !completedOnlyIds.Contains(id)).ToList();

        if (bylawPrereqLookup.Count > 0)
        {
            var gradedCourseIds = (await GradesRepo.GetAllAsync(
                new GradeSpec(studentId), asNoTracking: true))
                .Where(g => g.GradeType == GradeType.Final && g.Status == "Graded")
                .Select(g => g.CourseId)
                .ToHashSet();

            var completedPrereqIds = completedOnlyIds
                .Where(id => gradedCourseIds.Contains(id))
                .ToHashSet();

            availableCourseIds = availableCourseIds
                .Where(courseId =>
                {
                    if (!bylawPrereqLookup.TryGetValue(courseId, out var prereqs))
                        return true;

                    return prereqs.All(p =>
                        p.PrerequisiteCourse?.Course != null &&
                        completedPrereqIds.Contains(p.PrerequisiteCourse.CourseId));
                })
                .ToList();
        }
        ...
        var dataToReturn = courses.Select(c =>
        {
            ...
            if (electiveCourseIds.Contains(c.CourseId))
                dto.IsElective = true;
            if (bylawPrereqLookup.TryGetValue(c.CourseId, out var prereqs))
            {
                dto.Prerequisites = prereqs
                    .Select(p => p.PrerequisiteCourse?.Course?.CourseCode)
    ...
    public async Task<PaginatedResult<CoursePrerequisiteDto>> GetAllWithPrerequisitesByStudentBylawAsync(int studentId, CourseQueryParams queryParams)
    {
        ...
        var dataToReturn = courses.Select(c => new CoursePrerequisiteDto
        {
            ...
            Prerequisites = bylawPrereqLookup.TryGetValue(c.CourseId, out var prereqs)
                ? prereqs
                    .Select(p => p.PrerequisiteCourse)
                    .Where(p => p?.Course is not null)
                    .Select(p => new PrerequisiteItemDto
                    {
                        Code = p!.Course!.CourseCode ?? p.CourseId.ToString(),
                        CodeAr = p.Course.CourseCodeAr,
                        Title = p.Course.CourseName,
                        TitleAr = p.Course.CourseNameAr
                    })
                    .ToList()
                : []
        });
