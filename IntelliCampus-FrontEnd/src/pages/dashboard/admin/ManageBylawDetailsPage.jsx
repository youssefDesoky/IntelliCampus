...
export default function ManageBylawDetailsPage() {
  ...
  const [prereqSourceType, setPrereqSourceType] = useState(null);
  const [prereqSourceBucketId, setPrereqSourceBucketId] = useState(null);
  ...
  const closePrereqOverlay = () => {
    setPrereqTarget(null);
    setPrereqSourceType(null);
    setPrereqSourceBucketId(null);
    setPrereqSelectedIds([]);
    setPrereqSearchQuery("");
  };

  const openPrereqSelect = (courseId, sourceType, sourceBucketId) => {
    setPrereqTarget(courseId);
    setPrereqSourceType(sourceType);
    setPrereqSourceBucketId(sourceBucketId || null);
  ...
  const openBucketCourseSelect = async (bucketId) => {
    ...
    try {
      const deptsResult = await fetchDepartments();
      setBucketDepartments(Array.isArray(deptsResult?.data) ? deptsResult.data : []);
    } catch (err) {
  ...
  useEffect(() => {
    ...
    fetchDepartments().then((result) => {
      if (!cancelled) setBucketDepartments(Array.isArray(result?.data) ? result.data : []);
    }).catch(() => {});
  ...
  useEffect(() => {
    ...
    fetchDepartments().then((result) => {
      if (cancelled) return;
      const deptList = Array.isArray(result?.data) ? result.data : [];
  ...
  const handleSaveCourseMapping = async () => {
    ...
    try {
      // Validate course mapping hours before save
      const validationPayload = {
        courses: [
          ...universityRequired.map(c => ({
            courseId: c.courseId,
            courseType: "GeneralUniversity",
            creditHours: c.creditHours ?? null,
            allowedDepartmentIds: c.allowedDepartments?.length > 0 ? c.allowedDepartments : null,
          })),
          ...collegeRequired.map(c => ({
            courseId: c.courseId,
            courseType: "Faculty",
            creditHours: c.creditHours ?? null,
            allowedDepartmentIds: c.allowedDepartments?.length > 0 ? c.allowedDepartments : null,
        ...
        electiveBuckets: buckets.map(b => ({
          departmentId: b.departmentId || null,
          requiredCreditHours: b.requiredCreditHours || 0,
        })),
      };

      const validation = await validateCourseMapping(bylawId, validationPayload);
      if (!validation.isValid) {
        const failed = validation.departmentDetails.filter(d => !d.isValid);
        const failedDetails = failed.map(d =>
          `${d.departmentName} (mapped: ${d.calculatedTotalHours}h, required: ${d.requiredHours}h)`
        ).join('; ');
        throw new Error(
          `Insufficient total hours for: ${failedDetails}. ` +
          `Total hours to complete degree: ${validation.totalHoursToCompleteDegree}.`
        );
      }
      ...
      for (const entry of allCurrent) {
        ...
        if (bcId && entry.allowedDepartments?.length > 0) {
          await updateBylawCourseAllowedDepartments(bcId, { allowedDepartmentIds: entry.allowedDepartments });
        }
      }
  ...
  return (
    <div className="p-0 sm:p-6">
          ...
          {allCoursesLoading ? (
            ...
            <div className="space-y-6">
              <CourseMappingTable
                ...
                onSetPrerequisites={(id) => openPrereqSelect(id, "university", null)}
              ...
              <CourseMappingTable
                ...
                onSetPrerequisites={(id) => openPrereqSelect(id, "college", null)}
                onCreditHoursChange={handleCreditHoursChange}
                i18n={i18n}
              />
              <CourseMappingTable
                ...
                onSetPrerequisites={(id) => openPrereqSelect(id, "major", null)}
                          ...
                          {bucket.courseIds.map((item) => {
                            ...
                            return (
                              <div key={item.courseId} className="flex items-center gap-4 px-4 py-2.5">
                                ...
                                <button
                                  type="button"
                                  onClick={() => openPrereqSelect(item.courseId, "bucket", bucket.id)}
                                  className="p-1.5 rounded-lg text-text-accent-default-light dark:text-text-accent-default-dark hover:bg-blue-50 dark:hover:bg-blue-950/30 transition-colors"
                                  title={t('manageBylaws.setPrerequisitesTitle')}
                                >
                                  <LinkIcon size={16} />
      ...
      {prereqTarget && (
        <ModelOverlay onClose={closePrereqOverlay} maxWidth="max-w-2xl">
          <div className="w-full bg-bg-surface-primary-default-light dark:bg-bg-surface-primary-default-dark rounded-lg shadow-2xl flex flex-col max-h-[90vh]">
            {/* Header */}
            <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-border-primary-default-light dark:border-border-primary-default-dark">
              <h2 className="text-xl font-semibold text-text-primary-default-light dark:text-text-primary-default-dark">
              ...
              <button type="button" onClick={closePrereqOverlay} className="ms-auto p-1 text-icon-secondary-default-light dark:text-icon-secondary-default-dark hover:text-icon-secondary-hover-light dark:hover:text-icon-secondary-hover-dark transition-colors">
                <XIcon size={20} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-5 space-y-1 min-h-0 no-scrollbar">
              {allCourses
                ...
                .filter((c) => {
                  if (prereqSourceType === "university") {
                    return universityRequired.some(u => u.courseId === c.courseId);
                  }
                  if (prereqSourceType === "college") {
                    return universityRequired.some(u => u.courseId === c.courseId) ||
                      collegeRequired.some(co => co.courseId === c.courseId);
                  }
                  if (prereqSourceType === "major") {
                    return universityRequired.some(u => u.courseId === c.courseId) ||
                      collegeRequired.some(co => co.courseId === c.courseId) ||
                      majorRequired.some(m => m.courseId === c.courseId);
                  }
                  if (prereqSourceType === "bucket") {
                    const bucket = buckets.find(b => b.id === prereqSourceBucketId);
                    const bucketIds = new Set((bucket?.courseIds || []).map(ci => ci.courseId));
                    return bucketIds.has(c.courseId) ||
                      universityRequired.some(u => u.courseId === c.courseId) ||
                      collegeRequired.some(co => co.courseId === c.courseId) ||
                      majorRequired.some(m => m.courseId === c.courseId);
                  }
                  return true;
